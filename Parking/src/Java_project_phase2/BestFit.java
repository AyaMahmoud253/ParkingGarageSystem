			...
			System.out.println("Enter Number of cars that want to park in");
			 ...
			 Vehicle cars[]= new Vehicle[n];
				...
				for(int i=0;i<number;i++)
				{
					...
					if((i+1)<number)
					{
					miniw=Math.min(obj[i].getWidth(), obj[i+1].getWidth());
					minid=Math.min(obj[i].getDepth(), obj[i+1].getDepth());
					}
				}
		for(int i=0;i<number;i++)
		{
			...
			{
				for(int j=0;j<n;j++){		
			if((obj[i].getWidth()==cars[j].getWidth()&&obj[i].getDepth()==cars[j].getDepth())&&cars[j].getparked()==false)
			{
			v.parkIn(i, obj[i]);
			System.out.print("the Car "+cars[j].getID());
			System.out.println(" is in slot "+obj[i].getnum());
			...
			if((obj[i].getWidth()>=cars[j].getWidth()&&obj[i].getDepth()>=cars[j].getDepth())&&cars[j].getparked()==false)
			{
				if(obj[i].getWidth()==miniw||obj[i].getDepth()==minid)
				{
			v.parkIn(i, obj[i]);
			System.out.print("the Car "+cars[j].getID());
			System.out.println(" is in slot "+obj[i].getnum());
